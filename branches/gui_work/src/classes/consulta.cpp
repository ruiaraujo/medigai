...
Consulta::Consulta(Doctor *m , Patient *u ,const Date & d , const Hour & h , float p ) : med(m),  pac(u) , data(d), 
                                                        hora(h), duracao( m->getDur() ), preco(p) , estado('m') {}
Doctor * Consulta::getMed() const {return this->med;}
Patient * Consulta::getUte() const {return this->pac; }
...
void Consulta::setMed( Doctor * m ) { med = m; }
void Consulta::setUte( Patient * u ) { pac = u; }
...
{
  os << c.med->getId() << "|" << c.pac->getId() << "|" << c.data << "|" << c.hora << "|" << c.preco << "|" << c.duracao << "|" << c.estado;
  return os;
}
